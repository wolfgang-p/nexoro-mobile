...
class PhoneManager
{
   ...
   * Fehler festhalten.
   *
   * Der Wortlaut geht NUR ins Entwicklerprotokoll. In den Store kommt eine
   * schlichte Kennzeichnung, denn technische Meldungen — SIP-Ursachen,
   * Servernamen, Nebenstellen — gehen den Nutzer nichts an und verunsichern
   * ihn nur. Die Oberfläche zeigt stattdessen den Zustand des Telefons.
   */
  fehler(text)
  {
    this.protokoll(`FEHLER: ${ text }`);
    this.store.setFehler(true);
  }
   ...
   * Anruf annehmen.
   *
   * Wartet notfalls auf die SIP-Sitzung: Bei einem VoIP-Push zeigt CallKit
   * den Anruf an, sobald Apple die App geweckt hat — das INVITE des Asterisk
   * trifft erst ein bis zwei Sekunden spaeter ein. Nimmt der Nutzer in dieser
   * Luecke ab, gibt es noch nichts anzunehmen.
   *
   * Ohne dieses Warten klingelte es weiter, obwohl abgehoben wurde.
   */
  async annehmen()
  {
    if (!this.session)
    {
      this.protokoll('Annehmen: warte auf die SIP-Sitzung…');
      const bis = Date.now() + 8000;
      while (!this.session && Date.now() < bis)
      {
        // eslint-disable-next-line no-await-in-loop
    ...
    if (!this.session)
    {
      this.fehler('Annehmen: kein Anruf vorhanden');
      return;
    }
  ...
  _callKit()
  {
    ...
    callKitEinrichten({
      // BEWUSST OHNE Abgleich der Kennung.
      //
      // Bei einem VoIP-Push vergibt der Server eine eigene call_id; das
      // spaeter eintreffende SIP-INVITE erzeugt eine andere. Ein Vergleich
      // schlaegt deshalb immer fehl, und das Annehmen liefe ins Leere - das
      // Telefon klingelte weiter, obwohl der Nutzer abgehoben hat.
      //
      // Es kann ohnehin nur ein Gespraech gleichzeitig geben (siehe
      // anrufen()), also ist die Kennung entbehrlich.
      onAnnehmen: () => this.annehmen(),
      onBeenden: () => this.auflegen(),
  ...
  _verdrahte(session, richtung, anrufId, wer)
  {
    ...
    {
      this.protokoll(text);
      if (sichtbar) this.store.setFehler(true);
      this._toeneAus();
    ...
    {
      // Der Wortlaut ist fuer die Fehlersuche wertvoll, fuer den Nutzer
      // wertlos ("The m= section with mid='0' is invalid"). Deshalb nur ins
      // Protokoll.
