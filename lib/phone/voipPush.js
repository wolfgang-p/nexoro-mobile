...
async function eingehenderPush(nutzlast)
{
  ...
  //
  // Zeitlich heikel: CallKit klingelt bereits, das INVITE kommt aber erst,
  // wenn wir bei Kamailio angemeldet sind. Nimmt der Nutzer vorher ab, wartet
  // phoneManager.annehmen() auf die Sitzung — deshalb ist es hier wichtig,
  // dass die Anmeldung wirklich anläuft und nicht still scheitert.
  try
  {
    const zugang = await holeSipZugang();
    if (!zugang)
    {
      console.warn('[voip] keine Zugangsdaten - Anruf kann nicht angenommen werden');
      return;
    }
