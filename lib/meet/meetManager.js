...
class MeetManager
{
  ...
  async join({ roomId, identity, video = true, audioDeviceId })
  {
    ...
    // 7. Vorhandene Notizen laden. Ohne diesen Schritt stünde das Feld leer da,
    //    obwohl auf dem Server längst etwas liegt: die WS-Broadcasts liefern nur
    //    LAUFENDE Änderungen, nicht den Bestand. Wer später dazukommt oder das
    //    Panel neu öffnet, sah dadurch nichts — im Browser dagegen schon, weil
    //    der beim Öffnen des Panels lädt.
    try
    {
      const n = await meetings.getNotes(roomId, identity);
      if (typeof n?.content === 'string') store.setNotes(n.content);
    } catch (e) { /* Notizen sind nicht beitrittskritisch */ }

    // 8. Audio-Routing: Lautsprecher an, wenn Video läuft; sonst Hörmuschel.
  ...
  async startScreenShare()
  {
    ...
    // getDisplayMedia erwartet KEINE Media-Constraints wie getUserMedia. Das
    // Argument ist ein Options-Objekt mit plattformspezifischen Feldern; ein
    // {video:true} landete im nativen Modul als unbekannte Option und lieferte
    // einen Track ohne Bild — im Web sah man dann nur schwarz.
  ...
  async sendChat(body)
  {
    ...
    // NICHT zusätzlich broadcasten: der Server fächert die Nachricht bereits
    // über den WS an alle anderen aus (meetings/index.js → postMessage) und
    // lässt dabei den Absender aus. Ein eigenes Broadcast käme bei den anderen
    // ein zweites Mal an. Lokal anhängen genügt, damit sie sofort steht.
    if (res?.message) useMeetStore.getState().addChatMessage(res.message, { markRead: true });
  }

  /** Notizen frisch vom Server holen. Gibt null zurück, wenn es nicht klappt. */
  async loadNotes()
  {
    if (!this.roomId || !this.identity) return null;
    try
    {
      const n = await meetings.getNotes(this.roomId, this.identity);
      const content = typeof n?.content === 'string' ? n.content : null;
      if (content !== null) useMeetStore.getState().setNotes(content);
      return content;
    } catch (e)
    {
      return null;
